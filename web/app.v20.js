...
import { parseDicom } from "./dicom.v20.js";
import { draftReport } from "./report.v20.js";
import { decodeCompressed } from "./codecs/decode.v20.js";
import { triage as triageCtx } from "./triage.v20.js";
import { buildPackage, validatePackage, downloadJson } from "./consult.v20.js";
...
export async function seedDemo() {
  const mk = async (uid, patient, lesion, count) => {
    ...
    const study = {
      ...
      frames, triage: triageCtx(frames[0], parsed), created: Date.now(),
    };
...
async function exportConsult() {
  ...
  downloadJson(pkg, (isReply ? "rrai-reply-" : "rrai-consult-") + (current.patient || "study") + ".json");
  ...
  setStatus(isReply ? "صُدّر رد الاستشارة بحزمة موقّعة." : "صُدّرت حزمة الاستشارة — أرسل الملف بأي قناة آمنة.", false);
}
...
async function importConsult(file) {
  let pkg;
  try { pkg = JSON.parse(await file.text()); } catch (e) { setStatus("ملف غير مقروء — رُفض الاستيراد (فشل مغلق).", true); return; }
  ...
  let frames = [];
  if (p.snapshotPng) { try { frames = [await pngToFrame(p.snapshotPng)]; } catch (e) {} }
