...
function para(text, opts = {}) {
  const pPr = `<w:pPr>${opts.ltr ? "" : "<w:bidi/>"}<w:jc w:val="${opts.ltr ? "left" : "right"}"/></w:pPr>`;
  const rPr = `<w:rPr>${opts.bold ? "<w:b/>" : ""}${opts.sz ? `<w:sz w:val="${opts.sz}"/>` : ""}${opts.ltr ? '<w:rtl w:val="0"/>' : ""}</w:rPr>`;
