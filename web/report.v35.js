...
import { sha256Text } from "./consult.v35.js";
...
export function draftReport(study) {
  ...
  const ar = [
    ...
    ...(study.compare ? [`مقارنة مع ${study.compare.patient || study.compare.uid}: متوسط فرق ${Number(study.compare.meanAbs).toFixed(3)} ونقاط ساخنة ${Number(study.compare.pctHot).toFixed(1)}%.`] : []),
