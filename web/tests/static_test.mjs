...
const cp = spawnSync(process.execPath, ["--check", path.join(root, `app.${build}.js`), "--input-type=module-safe"], { input: fs.readFileSync(path.join(root, `app.${build}.js`)) });
const mparse = spawnSync(process.execPath, ["--check", "/dev/stdin"], { input: fs.readFileSync(path.join(root, `app.${build}.js`)) });
