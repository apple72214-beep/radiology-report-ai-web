...
import { parseDicom } from "./dicom.v24.js";
import { draftReport } from "./report.v24.js";
import { decodeCompressed } from "./codecs/decode.v24.js";
import { triage as triageCtx } from "./triage.v24.js";
import { buildPackage, validatePackage, downloadJson } from "./consult.v24.js";
...
function setupSignExport() {
  ...
  $("sign-export").addEventListener("click", async () => {
    ...
    if (!signer) { setStatus("التوقيع يتطلب اسم الطبيب — رُفض (فشل مغلق).", true); return; }
    ...
    try { await putStudy(current); } catch (e) {}
    $("print-root").innerHTML = doc.html;
    setStatus("فُتح حوار الطباعة — اختر «حفظ كـPDF». بصمة المستند: " + doc.hash.slice(0, 16) + "…", false);
    window.print();
  });
}
