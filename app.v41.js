...
const MODULES = [
  ["dicom", "./dicom.v41.js", (m) => { parseDicom = m.parseDicom; parseMultipartRelated = m.parseMultipartRelated; }],
  ["report", "./report.v41.js", (m) => { draftReport = m.draftReport; buildSignedDocument = m.buildSignedDocument; signedDocHash = m.signedDocHash; }],
  ["decode", "./codecs/decode.v41.js", (m) => { decodeCompressed = m.decodeCompressed; }],
  ["triage", "./triage.v41.js", (m) => { triageCtx = m.triage; }],
  ["consult", "./consult.v41.js", (m) => { buildPackage = m.buildPackage; validatePackage = m.validatePackage; downloadJson = m.downloadJson; }],
  ["docx", "./docx.v36.js", (m) => { buildDocxReport = m.buildDocxReport; buildDocxReportEn = m.buildDocxReportEn; }],
  ["measure", "./measure.v41.js", (m) => { segMm = m.segMm; angleDeg = m.angleDeg; ellipseStats = m.ellipseStats; }],
  ["compare", "./compare.v41.js", (m) => { cmpFrameStats = m.frameStats; normalize01 = m.normalize01; diffStats = m.diffStats; frameGrayFlat = m.frameGrayFlat; }],
];
...
function setupPacs() {
  ...
  if (dl) dl.addEventListener("click", async () => {
    const url = String($("pacs-url").value || "").trim();
    const st = $("pacs-status");
    const say = (t, bad) => { if (st) { st.textContent = t; st.style.color = bad ? "#fca5a5" : "#86efac"; } setStatus(t, !!bad); };
    if (!/^https?:\/\//i.test(url)) { say("ضع رابط التنزيل (WeTransfer أو ZIP أو DICOM مباشر) في حقل العنوان أولًا.", true); return; }
...
const PLACE_L = { "pacs-url": ["https://pacs.hospital.local/dicom-web", "https://pacs.hospital.local/dicom-web"], "pacs-q": ["معرف المريض (اختياري)", "Patient ID (optional)"] };
  ...
  $("gen-report").addEventListener("click", () => {
    ...
    (async () => {
      try {
        ...
        const hash2 = await signedDocHash(current, d, { signer: signer2, license: license2, at: at2 });
        const bytes2 = buildDocxReportEn(current, d, { signer: signer2 || "—", license: license2, org: org2, at: at2, hash: hash2 });
...
function setupSignExport() {
  ...
  if ($("sign-export-word") && !$("sign-export-word-en")) {
    ...
    ben.addEventListener("click", async () => {
      const s = await signCurrent();
      if (!s) return;
      const bytes = buildDocxReportEn(current, s.draft, { signer: s.signer, license: s.license, org: s.org, at: s.doc.at, hash: s.doc.hash });
