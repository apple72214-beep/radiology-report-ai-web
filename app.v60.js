...
function seriesFitFor() {
  ...
  let maxW = 0, maxH = 0;
  for (const fr of current.frames) { if (fr.w > maxW) maxW = fr.w; if (fr.h > maxH) maxH = fr.h; }
  const vp = $("viewport");
  const cw = (vp && (vp.clientWidth || Math.round(vp.getBoundingClientRect().width))) || 0;
  const ih = window.innerHeight || 800;
  let ch = 0;
  if (vp) { const vr = vp.getBoundingClientRect(); if (vr.top >= 0 && vr.top < ih) ch = ih - vr.top - 150; }
  const kw = cw ? cw / (maxW || 1) : 4;
  const kh = ch > 40 ? ch / (maxH || 1) : 4;
  return Math.max(0.25, Math.min(4, kw, kh));
}
