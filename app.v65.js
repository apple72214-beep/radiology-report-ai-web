...
function sliceFitFor(f) {
  ...
  const fs = !!document.fullscreenElement || !!(card && card.classList.contains("fs-fake"));
  if (fs || (window.matchMedia && window.matchMedia("(min-width: 900px)").matches)) {
    const ih = window.innerHeight || 800;
    let top = 0;
    if (vp && !fs) { const vr = vp.getBoundingClientRect(); top = Math.min(Math.max(vr.top, 0), ih * 0.5); }
    const ch = ih - top - (fs ? 170 : 150);
...
function show(idx) {
  ...
  $("meta").textContent =
    `${current.patient} • ${current.modality} • شريحة ${idx + 1} من ${current.frames.length} • ${current.description || ""} • ${APP_BUILD} • ملاءمة=${fitK.toFixed(2)}`;
