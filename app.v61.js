...
function seriesDims() {
  let maxW = 0, maxH = 0;
  if (current) for (const fr of current.frames) { if (fr.w > maxW) maxW = fr.w; if (fr.h > maxH) maxH = fr.h; }
  return [maxW, maxH];
}
function seriesFitFor() {
  if (!current || !current.frames.length) return 1;
  const d = seriesDims(), maxW = d[0], maxH = d[1];
  const vp = $("viewport");
  const cw = (vp && (vp.clientWidth || Math.round(vp.getBoundingClientRect().width))) || 0;
  const kw = cw ? cw / (maxW || 1) : 4;
  let k = kw;
  if (window.matchMedia && window.matchMedia("(min-width: 900px)").matches) {
    ...
    const ch = ih - top - 150;
    if (ch > 40) k = Math.min(k, ch / (maxH || 1));
  }
...
function resetView(fit) {
  ...
  const compute = () => {
    if (fit) fitK = seriesFitFor();
    vk = 1; vx = 0; vy = 0;
    const d = seriesDims();
    const stW = Math.max(1, Math.round((d[0] || f.w) * fitK));
    const stH = Math.max(1, Math.round((d[1] || f.h) * fitK));
    const wr = $("vwrap");
...
function show(idx) {
  ...
  $("meta").textContent =
    `${current.patient} • ${current.modality} • شريحة ${idx + 1} من ${current.frames.length} • ${current.description || ""} • ${APP_BUILD}`;
