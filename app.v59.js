...
function seriesFitFor() {
  ...
  let maxW = 0;
  for (const fr of current.frames) if (fr.w > maxW) maxW = fr.w;
  const vp = $("viewport");
  const cw = (vp && (vp.clientWidth || Math.round(vp.getBoundingClientRect().width))) || 0;
  return Math.min(4, (cw || maxW || 1) / (maxW || 1));
}
...
function show(idx) {
  ...
  $("meta").textContent =
    `${current.patient} • ${current.modality} • شريحة ${idx + 1} من ${current.frames.length} • ${current.description || ""} • v59`;
