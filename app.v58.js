...
const MODULES = [
  ["dicom", "./dicom.v58.js", (m) => { parseDicom = m.parseDicom; parseMultipartRelated = m.parseMultipartRelated; }],
  ["report", "./report.v58.js", (m) => { draftReport = m.draftReport; buildSignedDocument = m.buildSignedDocument; signedDocHash = m.signedDocHash; }],
  ["decode", "./codecs/decode.v58.js", (m) => { decodeCompressed = m.decodeCompressed; }],
  ["triage", "./triage.v58.js", (m) => { triageCtx = m.triage; }],
  ["consult", "./consult.v58.js", (m) => { buildPackage = m.buildPackage; validatePackage = m.validatePackage; downloadJson = m.downloadJson; }],
  ["docx", "./docx.v58.js", (m) => { buildDocxReport = m.buildDocxReport; buildDocxReportEn = m.buildDocxReportEn; }],
  ["measure", "./measure.v58.js", (m) => { segMm = m.segMm; angleDeg = m.angleDeg; ellipseStats = m.ellipseStats; }],
  ["compare", "./compare.v58.js", (m) => { cmpFrameStats = m.frameStats; normalize01 = m.normalize01; diffStats = m.diffStats; frameGrayFlat = m.frameGrayFlat; }],
];
    ...
    .then((t) => {
      ...
      if (m && m[1] !== APP_BUILD) {
        ...
        if (b) {
          ...
          b.innerHTML =
            'تعمل الآن على ' + APP_BUILD + ' — تحديث متاح (' + m[1] + '): <a href="?v=' + m[1] + '" style="color:var(--accent)">افتح النسخة المحدثة الآن</a> أو امسح بيانات الموقع من إعدادات Chrome.';
        }
      }
    })
    .catch(() => {});
...
function resetView(fit) {
  ...
  const compute = () => {
    ...
    fitK = fit && f ? Math.min(4, (cw || f.w) / f.w) : 1;
    vk = 1; vx = 0; vy = 0;
    const cv = $("frame");
...
function show(idx) {
  ...
  $("meta").textContent =
    `${current.patient} • ${current.modality} • شريحة ${idx + 1} من ${current.frames.length} • ${current.description || ""} • v58`;
