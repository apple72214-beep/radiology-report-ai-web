import { drawFrame } from "../app.v67.js";
let fails = 0;
...
const src = await import("node:fs").then((fs) => fs.readFileSync(new URL("../app.v67.js", import.meta.url), "utf-8"));
...
const sws = await import("node:fs").then((fs) => fs.readFileSync(new URL("../sw.v67.js", import.meta.url), "utf-8"));
ck(sws.includes("self.skipWaiting()") && sws.includes("clients.claim()"), "SW activates immediately and claims clients");
ck(sws.includes("app.v67.js"), "SW precache list matches current build");
...
ck(src.includes("const ch = vp ? (vp.clientHeight || 0) : 0;"), "fullscreen cover measured from real viewport height");
ck(src.includes("Math.max(cw / (f.w || 1), chh / (f.h || 1))"), "fullscreen uses COVER scale (fill screen, explore by drag)");
ck(src.includes("function fsArm()"), "tap reveals controls, auto-hide after 3s");
...
const ui = await import("node:fs").then((fs) => fs.readFileSync(new URL("../ui.v67.html", import.meta.url), "utf-8"));
...
ck(ui.includes("#viewer-card:fullscreen #viewport { position:absolute; inset:0; width:100%; height:100%; border-radius:0; }"), "fullscreen viewport = whole screen (image truly fills it)");
ck(ui.includes("#viewer-card:fullscreen.fs-hide .controls { opacity:0; pointer-events:none; }"), "controls auto-hide overlay in fullscreen");
...
console.log(fails ? "VIEWER TEST FAIL " + fails : "VIEWER TEST PASS (50)");
