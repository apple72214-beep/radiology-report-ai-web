import { drawFrame } from "../app.v63.js";
let fails = 0;
...
const src = await import("node:fs").then((fs) => fs.readFileSync(new URL("../app.v63.js", import.meta.url), "utf-8"));
...
const sws = await import("node:fs").then((fs) => fs.readFileSync(new URL("../sw.v63.js", import.meta.url), "utf-8"));
ck(sws.includes("self.skipWaiting()") && sws.includes("clients.claim()"), "SW activates immediately and claims clients");
ck(sws.includes("app.v63.js"), "SW precache list matches current build");
...
const ui = await import("node:fs").then((fs) => fs.readFileSync(new URL("../ui.v63.html", import.meta.url), "utf-8"));
...
console.log(fails ? "VIEWER TEST FAIL " + fails : "VIEWER TEST PASS (36)");
