import { drawFrame } from "../app.v55.js";
let fails = 0;
...
const src = await import("node:fs").then((fs) => fs.readFileSync(new URL("../app.v55.js", import.meta.url), "utf-8"));
...
ck(src.includes("(cx - r.left) * (1 - fct)"), "pinch zoom anchored at centroid");
ck(!src.includes("Math.max(1, Math.min(4,"), "fit allowed below 1x on phones");
ck(src.includes("• v55"), "build tag in meta line");
const ui = await import("node:fs").then((fs) => fs.readFileSync(new URL("../ui.v55.html", import.meta.url), "utf-8"));
ck(!ui.includes("image-rendering:pixelated"), "nearest-neighbor shimmer removed from viewer");
console.log(fails ? "VIEWER TEST FAIL " + fails : "VIEWER TEST PASS (15)");
