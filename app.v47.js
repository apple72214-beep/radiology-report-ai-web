...
const MODULES = [
  ["dicom", "./dicom.v46.js", (m) => { parseDicom = m.parseDicom; parseMultipartRelated = m.parseMultipartRelated; }],
  ["report", "./report.v46.js", (m) => { draftReport = m.draftReport; buildSignedDocument = m.buildSignedDocument; signedDocHash = m.signedDocHash; }],
  ["decode", "./codecs/decode.v46.js", (m) => { decodeCompressed = m.decodeCompressed; }],
  ["triage", "./triage.v46.js", (m) => { triageCtx = m.triage; }],
  ["consult", "./consult.v46.js", (m) => { buildPackage = m.buildPackage; validatePackage = m.validatePackage; downloadJson = m.downloadJson; }],
  ["docx", "./docx.v46.js", (m) => { buildDocxReport = m.buildDocxReport; buildDocxReportEn = m.buildDocxReportEn; }],
  ["measure", "./measure.v46.js", (m) => { segMm = m.segMm; angleDeg = m.angleDeg; ellipseStats = m.ellipseStats; }],
  ["compare", "./compare.v46.js", (m) => { cmpFrameStats = m.frameStats; normalize01 = m.normalize01; diffStats = m.diffStats; frameGrayFlat = m.frameGrayFlat; }],
];
...
export function aiRequestFor(key, model, parts) {
  return {
...
function aiPromptText(study) {
  ...
  return [
    ...
    "Rules: describe ONLY what the images and context support; never invent lesions, levels, or devices; if something is not assessable say so; keep sentences professional and concise; bold key anatomical terms with ** **; the draft is decision support, not a diagnosis.",
...
async function aiGenerate(study) {
  ...
  if (!okConsent) return;
  const parts = [{ text: aiPromptText(study) }];
  const n = study.frames.length;
  const idxs = n <= 3 ? study.frames.map((_, i) => i) : [0, Math.floor((n - 1) / 2), n - 1];
  for (const i of idxs) parts.push({ inline_data: { mime_type: "image/png", data: sliceRenderB64(study, i) } });
  say(LANG === "en" ? "AI drafting…" : "توليد مسودة بالذكاء…");
  let lastErr = null;
  for (const model of AI_MODELS) {
    try {
      const rq = aiRequestFor(key, model, parts);
      const r = await fetch(rq.url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(rq.body) });
      ...
      const txt = ((j.candidates || [])[0]?.content?.parts || []).map((pp) => pp.text || "").join("");
      ...
      if (current === study) { study.report = { exam: ex.value.trim(), indication: ind.value.trim(), findings: f.value.trim(), impression: im.value.trim() }; putStudy(study); }
...
function composerDefaults(study) {
  ...
  return { exam: examNameFor(study), indication: study.indication || "", findings: findings.join("\n"), impression: impLine.replace("Impression: ", "").trim() };
}
function readComposer() {
  const g = (id) => { const e = $(id); return e ? String(e.value || "").trim() : ""; };
  const exam = g("rep-exam"), indication = g("rep-indication"), f = g("rep-findings"), i = g("rep-impression");
  if (!exam && !indication && !f && !i) return null;
...
function setupComposer() {
  ...
  const save = () => {
    ...
    current.report = { exam: e.value.trim(), indication: ind.value.trim(), findings: f.value.trim(), impression: im.value.trim() };
    putStudy(current);
  };
  for (const id of ["rep-exam", "rep-indication", "rep-findings", "rep-impression"]) {
...
const I18N = {
  ...
  "lbl-ai-key": ["مفتاح Gemini API (يُحفظ على جهازك فقط)", "Gemini API key (stored on your device only)"],
  "ai-gen": ["توليد بمساعدة AI (مسودة للمراجعة)", "AI-assisted draft (for review)"],
  "ai-hint": ["اختياري وبإذنك لكل طلب: تُرسل 3 لقطات مصغّرة + إحصاءات رقمية فقط (لا ملفات DICOM الخام) إلى Google Gemini؛ المسودة تدخل لوحة التأليف قابلة للتحرير والتوقيع منك. المفتاح مجاني من Google AI Studio بلا بطاقة.", "Optional, per-request consent: only 3 downscaled renders + numeric stats (never raw DICOM) go to Google Gemini; the draft lands in the composer for you to edit and sign. Free key from Google AI Studio, no card."],
...
export async function init() {
  ...
  $("slice").addEventListener("input", (e) => { show(Number(e.target.value)); drawCompare(); });
  ...
  $("gen-report").addEventListener("click", () => {
    ...
    (async () => {
      try {
        ...
        el2.download = "rrai-report-EN-" + (current.patient || "study") + ".docx";
...
function setupSignExport() {
  ...
  if ($("sign-export-word") && !$("sign-export-word-en")) {
    ...
    ben.addEventListener("click", async () => {
      ...
      el.download = "rrai-report-EN-" + (current.patient || "study") + ".docx";
  ...
  $("sign-export-word").addEventListener("click", async () => {
    ...
    const el = document.createElement("a");
    el.href = URL.createObjectURL(blob);
    el.download = "rrai-report-" + (current.patient || "study") + ".docx";
    document.body.appendChild(el);
    el.click();
    setTimeout(() => { URL.revokeObjectURL(el.href); el.remove(); }, 400);
